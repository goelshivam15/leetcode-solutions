...
public:
    ...
    Node* f(int pre[] , int& i , int size,int mini, int maxi ){
        if(i>=size){
            return NULL;
        }
        ...
        nroot->data = pre[i++];
         nroot->left = f(pre,i,size,mini,nroot->data);
        nroot->right = f(pre,i,size,nroot->data,maxi);
